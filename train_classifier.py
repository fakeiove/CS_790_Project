...
def plan_generated_quota(real_kl3, real_kl4, avail_kl3, avail_kl4, total_budget):
    """
    Plan generated image counts (X for KL3, Y for KL4) under:
      1) X + Y <= total_budget
      2) Try to balance post-augmentation KL3/KL4 totals as much as possible.
    """
    if total_budget <= 0:
        return {3: 0, 4: 0}

    quota = {3: 0, 4: 0}
    avail = {3: max(0, int(avail_kl3)), 4: max(0, int(avail_kl4))}
    real = {3: int(real_kl3), 4: int(real_kl4)}

    for _ in range(int(total_budget)):
        can3 = quota[3] < avail[3]
        can4 = quota[4] < avail[4]
        if not can3 and not can4:
            break
        if can3 and not can4:
            quota[3] += 1
            continue
        if can4 and not can3:
            quota[4] += 1
            continue

        total3 = real[3] + quota[3]
        total4 = real[4] + quota[4]
        if total3 <= total4:
            quota[3] += 1
        else:
            quota[4] += 1
...
def main():
    ...
    parser.add_argument('--max_gen_ratio', type=float, default=0.5,
                        help='Max ratio of total generated images to total real training images. '
                             'E.g., 0.5 means generated <= 50% of all real train images. 0=no limit.')
    parser.add_argument('--k_values', nargs='+', type=float, default=None,
                        help='Optional sweep over K=(total_generated/total_real_train) for ldm/combined. '
    ...
    if args.k_values is not None:
        ...
        for k in k_sweep:
            if k <= 0 or k > 1:
                raise ValueError(
                    f"Invalid K in --k_values: {k}. For this codebase K=total_generated/total_real_train, "
                    "so K must be in (0, 1]."
                )
        print(f"K sweep enabled (K=total_generated/total_real_train): {k_sweep}")
    ...
    for plan in run_plan:
        ...
        if plan['k'] is not None:
            print(f"  K=total_generated/total_real_train={plan['k']:.4f} "
                  f"-> max_gen_ratio={max_gen_ratio:.4f}")
        print(f"{'='*60}")
        ...
        if exp_name == 'baseline':
            ...
            # Count real train images / KL3/4 for generated budget planning
            real_total = len(real_dataset)
            real_kl3 = int((train_df['v00_KL'] == 3).sum())
            real_kl4 = int((train_df['v00_KL'] == 4).sum())
            max_total_gen = (np.inf if max_gen_ratio == 0
                             else int(real_total * max_gen_ratio))

            # Build generated datasets first so we know true available counts
            raw_gen_datasets = []
            avail = {3: 0, 4: 0}
            for d in gen_info:
                ...
                raw_gen_datasets.append((d, gen_ds))
                avail[d['kl']] += len(gen_ds)

            if np.isinf(max_total_gen):
                class_quota = {3: avail[3], 4: avail[4]}
                print(f"  Max generated: no limit (ratio=0)")
            else:
                class_quota = plan_generated_quota(
                    real_kl3=real_kl3,
                    real_kl4=real_kl4,
                    avail_kl3=avail[3],
                    avail_kl4=avail[4],
                    total_budget=max_total_gen
                )
                print(f"  Real train images: {real_total}, generated budget <= {max_total_gen} "
                      f"(ratio={max_gen_ratio}x real_total)")
                print(f"  Planned generated quota: KL3={class_quota[3]}, KL4={class_quota[4]} "
                      f"(total={class_quota[3] + class_quota[4]})")
                print(f"  Post-augmentation KL3/KL4 target totals: "
                      f"{real_kl3 + class_quota[3]} / {real_kl4 + class_quota[4]}")

            remaining_quota = class_quota.copy()
            gen_datasets = []
            total_gen = 0
            for d, gen_ds in raw_gen_datasets:
                take = min(len(gen_ds), remaining_quota[d['kl']])
                if take <= 0:
                    print(f"  + {d['name']}: skipped by quota (KL{d['kl']})")
                    continue
                if take < len(gen_ds):
                    gen_ds.image_paths = gen_ds.image_paths[:take]
                    print(f"  + {d['name']}: capped to {take} images (KL{d['kl']})")
                else:
                    print(f"  + {d['name']}: {take} generated KL{d['kl']} images")
                remaining_quota[d['kl']] -= take
